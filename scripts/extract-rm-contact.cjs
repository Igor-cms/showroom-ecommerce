...
const BASE = "https://readymag.website/u2412057466/6396534/6/";
...
(async () => {
  ...
  const target = null || burger || { x: 1564, y: 30 }; // fallback: mockup burger position
  await page.mouse.click(target.x, target.y);
  await new Promise((r) => setTimeout(r, 2500));
  await page.screenshot({ path: DIR + "contact-rm.png" });
  ...
  fs.writeFileSync(DIR + "contact-dump.json", JSON.stringify(dump, null, 1));
