...
/* =========================================================================
 *  /blog — editorial grid of blog posts.
 *
 *  Mirrors the /podcast layout: standard Native header, a navigation band
 *  (big "NATIVE BLOG" title + year filters + horizontal search), then a 3-up
 *  grid of large post cards with thin dividers. Placeholder data — reusing the
 *  same imagery as the podcast — until real posts exist. Same off-white / ink,
 *  minimalist premium aesthetic as the rest of the site.
 * ========================================================================= */
...
/* Placeholder posts — same photos as the podcast. Replace `href` with real
   article links when available. */
const POSTS: Post[] = [
  {
    id: "post-12",
    year: 2026,
    date: "MAR 04, 2026",
    title: "INSIDE THERMAL SHOCK PROCESSING",
    description:
      "A field note from Popayán on how controlled fermentation and thermal shock reshape a coffee's flavour.",
  ...
  },
  {
    ...
    year: 2026,
    date: "FEB 18, 2026",
    title: "THE FAMILIES BEHIND THE LOT",
    description:
      "Traceability isn't a label — it's the people. A look at the community that stands behind every harvest.",
  ...
  },
  {
    ...
    year: 2026,
    date: "FEB 02, 2026",
    title: "BUILDING A ROAST PROFILE",
    description:
      "Inside the Dallas roastery: how a profile is built to honour — not flatten — a producer's work.",
  ...
  },
  {
    ...
    description:
      "An honest essay on pricing, hype and the gap between marketing and the farm gate.",
  ...
  },
  {
    ...
    year: 2026,
    date: "JAN 07, 2026",
    title: "NOTES ON THE FIRST CUP",
    description:
      "Producers and roasters share the earliest coffee memories that turned a craft into a calling.",
  ...
  },
  {
    ...
    year: 2026,
    date: "DEC 17, 2025",
    title: "WHY WE KEEP PUSHING BOUNDARIES",
    description:
      "Experimentation is a responsibility, not a gimmick — and what the next generation stands to inherit.",
  ...
  },
  {
    ...
    year: 2027,
    date: "MAR 22, 2027",
    title: "DESIGNING THE SHOWROOM",
    description:
      "How the roastery + showroom model carries the producer's story all the way to the cup in your hand.",
...
/* Every cover, hoisted so the array identity is stable and the preload runs
   ...
   thumbnails. */
const POST_IMAGES = POSTS.map((p) => p.image);
...
const Blog = () => {
  ...
  const imagesReady = useImagesReady(POST_IMAGES);

  /* Mobile has no year filter / search band, so it lists every post. */
  const posts = useMemo(() => {
    if (isMobile) return POSTS;
    const q = query.trim().toLowerCase();
    return POSTS.filter((p) => p.year === year).filter(
      (p) =>
        !q ||
    ...
    <div className="min-h-screen bg-bg-cream text-ink">
      ...
      <main style={{ paddingTop: "var(--site-header-h, 72px)" }}>
        {/* ── Navigation band: title + year filters + search (desktop only) ──
            Metrics read off the Readymag desktop canvas (1024 wide, preview/8):
        ...
        {/* ── Post grid: 3-up on desktop, thin dividers between cards ── */}
        {posts.length > 0 ? (
          <div className="grid grid-cols-1 gap-px bg-line sm:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => (
              <article key={post.id} className="flex flex-col bg-bg-cream">
                {/* Square at every width. The Readymag canvas actually draws
                    the cover 343×318 on desktop, but the client overrides its own
